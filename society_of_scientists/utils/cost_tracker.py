...
class ModelPricing:
    ...
    jamba_large_1_7: Dict[str, float] = field(
    ...
    jamba_mini_2: Dict[str, float] = field(
        default_factory=lambda: {
            "prompt": 0.0002,
            "completion": 0.0004,
        }
    ...
    jamba_mini_1_7: Dict[str, float] = field(
        default_factory=lambda: {
            "prompt": 0.0002,
            "completion": 0.0004,
        }
    ...
    jamba_large: Dict[str, float] = field(
        default_factory=lambda: {
            "prompt": 0.002,
            "completion": 0.008,
        }
...
class CostTracker:
    ...
    def __init__(
        self, log_file: Optional[str] = None, budget_usd: Optional[float] = None
    ):
        """
    ...
    def get_usage_stats(self) -> Dict:
        ...
        if not self.usage_history:
            return {
                ...
                "budget": {
                    ...
                    "spent": 0.0,
                    "remaining": self._budget_usd,
                    "utilization": 0.0 if self._budget_usd else None,
                },
            }
        ...
        for usage in self.usage_history:
            if usage.model not in by_model:
                by_model[usage.model] = {"calls": 0, "cost": 0.0, "tokens": 0}
        ...
        return {
            ...
            "budget": {
                ...
                "remaining": round(self._budget_usd - total_cost, 6)
                if self._budget_usd
                else None,
                "utilization": round((total_cost / self._budget_usd * 100), 2)
                if self._budget_usd
                else None,
            },
        }
    ...
    def get_budget_remaining(self) -> Optional[float]:
        ...
        if self._budget_usd is None:
            return None
        return self._budget_usd - self.get_total_cost()
    ...
    def get_budget_utilization(self) -> Optional[float]:
        ...
        if self._budget_usd is None or self._budget_usd == 0:
            return None
        return self.get_total_cost() / self._budget_usd * 100

    def check_budget(self, additional_cost: float = 0) -> tuple[bool, Optional[str]]:
        """Check if we're within budget limits."""
        if self._budget_usd is None:
            return True, None

        spent = self.get_total_cost()
        total_after = spent + additional_cost

        if total_after > self._budget_usd:
            return (
                False,
                f"Budget exceeded: spent ${spent:.6f}, budget ${self._budget_usd:.6f}",
            )

        utilization = (total_after / self._budget_usd) * 100
        if utilization >= 90:
            return (
                True,
                f"Budget warning: used {utilization:.1f}% of ${self._budget_usd:.6f}",
            )
        elif utilization >= 75:
            return (
                True,
                f"Budget notice: used {utilization:.1f}% of ${self._budget_usd:.6f}",
            )
    ...
    def check_budget_warnings(self) -> List[str]:
        ...
        if self._budget_usd:
            ...
            if utilization >= 95:
                warnings.append(
                    f"CRITICAL: Budget nearly exhausted ({utilization:.1f}% used)"
                )
            elif utilization >= 90:
                warnings.append(
                    f"WARNING: Over 90% of budget used ({utilization:.1f}%)"
                )
            elif utilization >= 75:
                warnings.append(f"NOTICE: Over 75% of budget used ({utilization:.1f}%)")
        return warnings
    ...
    def _load_history(self):
        ...
        if os.path.exists(self.log_file):
            try:
                ...
                self.usage_history = [APIUsage(**item) for item in data]
            except Exception as e:
    ...
    def print_summary(self):
        ...
        if total_cost < 0.000001:
            print(
                f"Total Cost: ${total_cost:.10f} (${total_cost * 1000:.6f} per 1K tokens)"
            )
